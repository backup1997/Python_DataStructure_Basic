...
def orderByAvail(node: Vertex):
    resList = []
    for v in node.getConnections():
        if v.getColor == "White":
            c = 0  # 用以计数v节点可以再接着合法移动的个数
            for w in v.getConnections():
                if w.getColor == "White":
                    c = c + 1
            resList.append((v, c))
    resList.sort(key=lambda x: x[1])  # 依据c进行排序
    return [y[0] for y in resList]
